...
def leader_board():
    ...
    for row1 in connection.execute("SELECT username,teacher,admin,sprogress FROM LoginDetails ORDER BY sprogress DESC LIMIT 5"):
        ...
        # If the person is a student
            ...
            row1 = str(new_row1)
            # Replace all unnecessary formatting symbols to nothing (essentially removing them for lack of a better method)
            row1 = row1.replace("[","")
            row1 = row1.replace("]","")
            row1 = row1.replace("'","")
...
def submit_register():
    ...
    if new_username_field.get() and new_password_field.get():
        ...
        hash.update(('%s' % (new_password)).encode('utf-8'))
        ...
        if temp[0] == "":
            ...
            # If all of these pass
                ...
                connection.execute("INSERT INTO LoginDetails(username,password,teacher,admin,sprogress,gprogress,ascore) VALUES (?,?,0,0,0,0,0)",(new_username, password_hash,))
...
def submit_login():
    ...
    if login_username_field.get() and login_password_field.get():
        ...
        hash.update(('%s' % (LOGIN_PASSWORD)).encode('utf-8'))
        ...
        for row in connection.execute("SELECT * FROM LoginDetails WHERE username = ? AND password = ?",(LOGIN_USERNAME, password_hash)):
        ...
        # If they are a student 
            ...
            for row in connection.execute("SELECT sprogress FROM LoginDetails WHERE username = ? AND password = ?",(LOGIN_USERNAME, password_hash)):
                # Set the variable to the progress
                SPROGRESS = row
            # Set the other global variable to string
            GPROGRESS = ""
            # Get the German progress of the student
            for row in connection.execute("SELECT gprogress FROM LoginDetails WHERE username = ? AND password = ?",(LOGIN_USERNAME, password_hash)):
                # Set the variable to the progress
                GPROGRESS = row
...
# This is the main menu function which is called initially and when logging out, which removes all labels that may be in the way 
def main_window():
    ...
    # Moves all the unnecessarry labels, buttons and fields out of the screen
...
def create_spanish_combo_box():
    ...
    # Set the deafult option to a Tkinter String Variable in the main window
    ...
    # Set the state to read only to avoide user's typing into box
...
def german_learn_mode():
    ...
    # Move unneccessary items off screen
    logout_button.place(x=3000, y=3000)
    german_zone_label.place(x=3000, y=3000)
    german_arcade_button.place(x=3000, y=3000)
    ...
    if german_learn_entry.get():
        ...
        # If their answer is wrong
            ...
            # If they are in learn mode
                ...
                # Move unneccessary items out of the way
...
def countdown():
    ...
    if ARCADE_TIMER >= -1:
        ...
        if ARCADE_TIMER >= 0:
            ...
            TIMER_COUNTER = Label(main_gui, text = timer_label, font=("Lucida Sans", 10, "bold"), bd = 3, bg = "orange", fg = "white")
...
def german_arcade_mode():
    ...
    # Begin the countdown function
    countdown()
    # After 1 minute + 0.5 seconds for delays call the main learn fuction
    main_gui.after(60500,main_learn)
...
def reset_password_process():
    ...
    if reset_password_field.get() and id_field.get():
        ...
        hash.update(('%s' % (reset_password)).encode('utf-8'))
...
def submit_question_set():
    ...
    global ADD_QUESTIONS_LABEL
    global DONE_ADD_QUESTIONS_LABEL
    # If the name field has been inputted and either the Spanish module or the German module checkbutton has been clicked
...
# This is the remove question sets procress
