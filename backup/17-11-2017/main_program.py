...
def main_window():
    ...
    module_combobox.place(x=3000, y=3000)
...
def spanish_modules():
    ...
    module_combobox.place(x=100, y=200)
...
def remove_user_process():
    ...
    if id_field.get():
        ...
        if temp == "()":
            ...
            connection.commit()
            done_remove_text = "User has been removed."
...
question_connection_cursor.execute("SELECT * FROM QuestionSets")
values = question_connection_cursor.fetchall()

for value in values:
     data = "%s %s %s" % (value["id"], value["name"], value["words"])
     question_question_sets.append(data)

question_connection.commit()
question_connection_cursor.close()

module_default = StringVar(main_gui)
try:
    module_default.set(question_sets[0])
except IndexError:
    module_default.set("No question sets found.")

module_combobox = ttk.Combobox(main_gui, textvariable=module_default, values=question_sets, width=60)
module_combobox.place(x=3000, y=3000)
